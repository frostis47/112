...
def filter_by_currency(transactions: list, currency_code: str) -> Iterator:
    ...
    for transaction in transactions:
        if currency_code == transaction["operationAmount"]["currency"]["code"]:
            yield transaction  # Возвращаем всю транзакцию
...
def card_number_generator(start: int, end: int) -> Iterator:
    ...
    for i in range(start, end + 1):
        cards_number = '{:04d} {:04d} {:04d} {:04d}'.format(
            (i // 10 ** 12) % 10 ** 4,
            (i // 10 ** 8) % 10 ** 4,
            (i // 10 ** 4) % 10 ** 4,
            i % 10 ** 4
...
# Пример использования для получения описаний транзакций
